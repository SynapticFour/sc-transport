...
async fn loopback_throughput_is_nonzero() {
    common::with_timeout("loopback_throughput_is_nonzero", 120, async {
        ...
        tokio::fs::write(&src, vec![0x33_u8; PAYLOAD_LEN])
            .await
            .expect("write");
        ...
        let throughput_mbps = ((PAYLOAD_LEN as f64) * 8.0 / 1_000_000.0) / elapsed;
...
async fn resume_stress_multiple_iterations() {
    common::with_timeout("resume_stress_multiple_iterations", 300, async {
        for _ in 0..3 {
            let recv_tmp = tempfile::tempdir().expect("tempdir");
            std::env::set_var("HOME", recv_tmp.path());
            let send_tmp = tempfile::tempdir().expect("tempdir");
...
async fn multi_file_dataset_transfers() {
    common::with_timeout("multi_file_dataset_transfers", 180, async {
        let recv_tmp = tempfile::tempdir().expect("tempdir");
        std::env::set_var("HOME", recv_tmp.path());
        let send_tmp = tempfile::tempdir().expect("tempdir");
        let files = vec![
